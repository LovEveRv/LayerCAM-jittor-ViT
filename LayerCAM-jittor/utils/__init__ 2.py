...
# from .imagenet import *  # I have to remove this line to make it work
...
# MODIFIED: add 'cmap' to gradient subplot
def visualize(input_, gradients, save_path=None, cmap='viridis', alpha=0.7):
    ...
    subplots = [('Input image', [(input_, None, None)]), ('Saliency map across RGB channels', [(gradients, cmap, None)]), ('Overlay', [(input_, None, None), (gradients, cmap, alpha)])]
...
def basic_visualize(input_, gradients, save_path=None, weight=None, cmap='viridis', alpha=0.7):
    input_ = format_for_plotting(denormalize(input_))
    gradients = format_for_plotting(standardize_and_clip(gradients))
    subplots = [('Saliency map across RGB channels', [(gradients, cmap, None)]), ('Overlay', [(input_, None, None), (gradients, cmap, alpha)])]
    num_subplots = len(subplots)
...
# NEW: my new function
def find_vit_layer(arch, target_layer_name):
    target_layer = arch._modules['blocks'][target_layer_name]
    return target_layer
